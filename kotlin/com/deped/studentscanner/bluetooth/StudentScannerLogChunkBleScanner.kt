...
class StudentScannerLogChunkBleScanner(private val context: Context) {
    companion object {
        private const val TAG = "StudentScannerLogChunkBleScanner"
    ...
    private var onLogReceived: ((Int, String) -> Unit)? = null
    private var scanCallback: ScanCallback? = null

    fun startScan(onLogReceived: (logId: Int, logJson: String) -> Unit, onProgress: (String) -> Unit, onError: (String) -> Unit) {
        if (isScanning) {
        ...
        scanCallback = object : ScanCallback() {
            override fun onScanResult(callbackType: Int, result: ScanResult?) {
                ...
                if (logChunks.size == total && total > 0) {
                    ...
                    onLogReceived?.invoke(logId, logJson)
                    stopScan()
                }
            }
